...
async function getPlusServerData(product) {
    ...
    return servers.filter(server => server['server'].substring(1, 3) === `${product}` && server['server'].substring(0, 1) === 'S' && server['server'].substring(3, 4) === 'S');
}
...
async function getNeededServers(product) {
    ...
    return neededServersWithoutVersion.filter(neededServers => neededServers.substring(1, 3) === `${product}` && neededServers.substring(0, 1) === 'S' && neededServers.substring(3, 4) === 'S');
}

Promise.all([getPlusServerData(35), getNeededServers(35)]).then(result => {
